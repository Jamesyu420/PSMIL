...
vio <- as.matrix(read.csv("violated.csv", header = F))

data <- apply(vio,2,sum)
names(data) <- c(50, 100, 200, 500, 1000)
barplot(data, xlab = "N", main = "Number of violations")
...
msedata <- rbind(log(Err.ins[,2:5]), log(Err.bag[,2:5]), log(Err.com[,2:5]))
...
ggplot(data=msedata, aes(x=N, y=value, fill=Estimator)) + 
  geom_boxplot_pattern(
    ...
    fill = 'gray'  # 设置填充颜色为灰色，所有箱线图颜色一致
  ) + 
  labs(x="N", y="log(MSE)") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 20),  # 调整标题的大小
    axis.title = element_text(size = 16),  # 调整坐标轴标题的大小
    axis.text = element_text(size = 14),  # 调整坐标轴标签的大小
