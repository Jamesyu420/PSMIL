...
import numpy as np
# Data generating process with spatial clustering
def DataGen(mu1, mu0, Sigma, N, M, pis, lambda0, seed=0):
    p = len(mu1)
    np.random.seed(seed)
    
    # Step 1: Generate random distribution in a circle using polar coordinates
    radius = np.sqrt(np.random.rand(N, M))                                   # Uniform distribution in unit circle
    angle = np.random.rand(N, M) * 2 * np.pi                                 # Uniform angle
    S = np.stack((radius * np.cos(angle), radius * np.sin(angle)), axis=-1)  # Spatial location S

    # Step 2: Define the generating probability of A as now * np.sin(S[0])*np.sin(S[1])
    sin_product = np.sin(S[..., 0]) * np.sin(S[..., 1])
    normalized_sin_product = 0.8 + 0.2 * (sin_product - np.min(sin_product)) / (np.max(sin_product) - np.min(sin_product))
    # Here we normalize the product to [0.8,1] to restrict its variability 
    adjusted_pis = pis.reshape([-1, 1]) * normalized_sin_product  # Now pis
    Y = 1 * (np.random.rand(N) < lambda0)   
    A = 1 * (np.random.rand(N, M) < adjusted_pis)
    A[Y == 0] = 0 

    # Step 3: Generate X as before
...
save_dir = "/teams/WSIresult_1727165526/RobustResult/Study2/"
